...
module.exports = function Email(sails) {
  ...
  return {
    ...
    send: function (template, data, options, cb) {
      ...
      emailTemplate.render(data, function(err, result){
        if(err){
          return cb(err);
        }

        defaultOptions.html = result.html;
        if (result.text) defaultOptions.text = result.text;
        if (result.subject) defaultOptions.subject = result.subject;
        ...
        transport.sendMail(mailOptions, function(err, results){
          if(err){
            return cb(err);
          }
