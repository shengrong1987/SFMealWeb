...
module.exports = function(){
    ...
    findOrCreateAuth: function(criteria, attributes, cb){
      var self = this;
      var authCriterias = ['email','facebookId','googleEmail','password'];
      var authAttr = {};
      authCriterias.forEach(function(key){
        if(attributes.hasOwnProperty(key)){
          authAttr[key] = attributes[key];
        }
      })
      waterlock.Auth.findOrCreate(criteria, authAttr)
      .exec(function(err, newAuth){
        ...
        .exec(function(err, auth){
          ...
          self._attachAuthToUser(auth, function(err, user){
            if(err){
              return cb(err);
            }
            waterlock.User.cloneToUser(user, attributes, cb);
          });
        });
      });
    },
