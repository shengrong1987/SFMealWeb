...
var Agenda = module.exports = function(config, cb) {
  ...
  this._lockedJobs = [];
  this._jobQueue = [];
  this._defaultLockLifetime = config.defaultLockLifetime || 1 * 60 * 1000; //10 minute default lockLifetime
...
Agenda.prototype.jobs = function( query, cb ){
  var self = this;
  var skip = (!query.skip || typeof query.skip === "undefined") ?  0 : query.skip;
  var limit = (!query.limit || typeof query.limit === "undefined") ?  0 : query.limit;
  delete query.skip;
  delete query.limit;
  this._collection.find(query).skip(parseInt(skip)).limit(parseInt(limit)).toArray( function( error, result ){
    var jobs;
    if( !error ){
...
Agenda.prototype.every = function(interval, names, data, options, cb) {
  ...
  function createJob(interval, name, data, options, cb) {
    ...
    job.attrs.lockedAt = self._notLocked;
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  // console.log("lock at: " + lockDeadline + "lockLifetime: " + definition.lockLifetime);
