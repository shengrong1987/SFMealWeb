...
module.exports = function(req, res) {
	...
	function accessTokenResponse(error, result) {
      ...
      userFound(null, req.session.user);
		} else {
      if(attr.email){
        waterlock.engine.findAuth({ email : attr.email}, function(err, user) {
          if (user && user.auth && !user.auth.googleEmail) {
            waterlock.cycle.loginFailure(req, res, null, {error: 'User already exists'});
          }else{
