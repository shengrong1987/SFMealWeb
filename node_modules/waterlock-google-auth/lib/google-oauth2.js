...
GoogleOAuth2.prototype._getMeCallback = function(err, response, body){
  ...
  if(!this.accessToken.id_token){
    return this._tokenCallback({status: 403, message: 'access token lack of "id_token"'});
  }
...
GoogleOAuth2.prototype.decryptRawToken = function(rawToken){
  ...
  sails.log.info("alg: " + token.alg);
  sails.log.info("kid: " + token.alg.kid);
...
GoogleOAuth2.prototype.getKey = function(kid){
  ...
  for(var i = 0; i < keys.length; i++) {
    sails.log.info("key: " + keys[i] + ", with kid: " + keys[i].kid);
    if(keys[i].kid === kid) {
      return keys[i];
    }
  }
...
GoogleOAuth2.prototype.buildPublicKey = function(key) {
  sails.log.info("key: " + key, ", key.use: " + key.use);
