...
describe('MealController', function() {
  ...
  describe('build a meal with dishes', function() {
    ...
    it('should create an order type meal ', function (done) {
      ...
      agent
          .post('/meal')
          .send({provideFromTime: now, provideTillTime: new Date(now.getTime() + 3600 * 600 * 5), leftQty: leftQty, totalQty: totalQty, county : 'San Francisco County', title : "私房面馆", type : "order", dishes : dishes, status : "on",cover : dish1, minimalOrder : 1})
          .expect(200)
          .end(function(err,res){
    ...
    it('should create an preorder type meal ', function (done) {
      ...
      agent
          .post('/meal')
          .send({provideFromTime: now, provideTillTime: new Date(now.getTime() + 3600 * 60 * 5), pickupFromTime : new Date(now.getTime() + 3600 * 60 * 5), pickupTillTime : new Date(now.getTime() + 3600 * 60 * 7),  leftQty: leftQty, totalQty: totalQty, county : 'San Francisco County', title : "私房面馆", type : "preorder", dishes : dishes, status : "on", cover : dish1, minimalOrder : 1})
          .expect(200)
          .end(function(err,res){
            if(res.body.chef != hostId){
              return done(Error("error creating meal"));
