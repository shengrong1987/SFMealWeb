...
export default {
  ...
  getBadge : function(id){
    $.ajax({
      ...
      ActionCreators.getBadge(data);
    }).fail(function(jqXHR, textStatus){
      ActionCreators.noResult(jqXHR.responseText);
    });
  },

  getBadges : function(criteria, value, skip){
    if(value) {
      var url = "/badge?sort=createdAt DESC&skip=" + skip + "&" + criteria + "=" + value;
    }else{
      url = "/badge?sort=createdAt DESC&skip=" + skip;
    }
    $.ajax({
      url: url,
      type: 'GET',
