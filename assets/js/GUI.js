...
var refreshMenu = function(){
  ...
  if(coupons.length > 0){
    ...
    var total = subtotal+delivery+tax+transactionFee-discount;
    if(total < 0){total = 0;}
    ...
    total = (subtotal+delivery+tax+serviceFee+transactionFee).toFixed(2);
  ...
  if(localPoints){
    ...
    total = subtotal+delivery+tax+transactionFee-discount;
    if(total < 0){total = 0;}
    total = (total + serviceFee).toFixed(2);
    $orderTotal.data("value",total);
    $orderTotal.html(" $" + total + "( -$" + discount.toFixed(2) + " )");
